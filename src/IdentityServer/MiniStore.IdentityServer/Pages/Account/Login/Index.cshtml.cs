...
namespace MiniStore.IdentityServer.Pages.Account.Login;
...
public class Index : PageModel
{
    ...
    public async Task<IActionResult> OnPostAsync(CancellationToken ct)
    {
        ...
        AuthorizationRequest? context = await _interaction.GetAuthorizationContextAsync(Input.ReturnUrl, ct);
        ...
        if (ModelState.IsValid)
        {
            ...
            bool rememberLogin = LoginOptions.AllowRememberLogin && Input.RememberLogin;

            SignInResult result = await _signInManager.PasswordSignInAsync(Input.Username!, Input.Password!, rememberLogin, true);
            if (result.Succeeded)
            {
    ...
    private async Task BuildModelAsync(string? returnUrl, CancellationToken ct)
    {
        ...
        IEnumerable<ViewModel.ExternalProvider> dynamicSchemes = (await _identityProviderStore.GetAllSchemeNamesAsync(ct))
            .Where(x => x.Enabled)
            .Select(x => new ViewModel.ExternalProvider
            (
                x.Scheme,
